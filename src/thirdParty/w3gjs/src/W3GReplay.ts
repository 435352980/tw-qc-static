...
class W3GReplay extends EventEmitter {
  ...
  constructor() {
    ...
    this.parser.on('basic_replay_information', (information: BasicReplayInformation) => {
  ...
  async parse($buffer: string | Buffer): Promise<ParserOutput> {
    this.msElapsed = 0;
    this.parseStartTime = moment.now();
    ...
    await this.parser.parse(Buffer.from($buffer));
    ...
    this.slots.forEach(slot => {
      if (slot.slotStatus > 1) {
        ...
        this.players[slot.playerId] = new Player(
          slot.playerId,
          tempPlayers[slot.playerId] ? tempPlayers[slot.playerId].playerName : 'Computer',
          slot.teamId,
          slot.color,
  ...
  handleActionBlock(action: Action, currentPlayer: Player): void {
    switch (action.id) {
      case 0x10:
        ...
        currentPlayer.handle0x10(objectIdFormatter(action.itemId), this.totalTimeTracker);
        break;
      case 0x11:
        currentPlayer.handle0x11(objectIdFormatter(action.itemId), this.totalTimeTracker);
        break;
      case 0x12:
  ...
  finalize(): ParserOutput {
    ...
    const root = {
      ...
      settings,
      parseTime: Math.round(moment.now() - this.parseStartTime),
    };
    return root;
  }
}
