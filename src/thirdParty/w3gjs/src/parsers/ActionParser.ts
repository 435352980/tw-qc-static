...
export default class ActionParser extends StatefulBufferParser {
  ...
  private parseAction(actionId: number): Action | null {
    switch (actionId) {
      ...
      case 0x10: {
        ...
        const itemId = [this.readUInt8(), this.readUInt8(), this.readUInt8(), this.readUInt8()];
      ...
      case 0x14: {
        const abilityFlags = this.readUInt16LE();
        const itemId1 = [this.readUInt8(), this.readUInt8(), this.readUInt8(), this.readUInt8()];
        const unknownA = this.readUInt32LE();
        const unknownB = this.readUInt32LE();
        const targetAX = this.readFloatLE();
        const targetAY = this.readFloatLE();
        const itemId2 = [this.readUInt8(), this.readUInt8(), this.readUInt8(), this.readUInt8()];
        this.skip(9);
  ...
  private readSelectionUnits(length: number): { itemId1: number[]; itemId2: number[] }[] {
    const v: { itemId1: number[]; itemId2: number[] }[] = [];
    for (let i = 0; i < length; i++) {
      const obj = {
        itemId1: [this.readUInt8(), this.readUInt8(), this.readUInt8(), this.readUInt8()],
        itemId2: [this.readUInt8(), this.readUInt8(), this.readUInt8(), this.readUInt8()],
      };
      v.push(obj);
    }
    return v;
